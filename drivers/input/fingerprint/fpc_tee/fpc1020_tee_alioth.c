...
static int reset_gpio_res(struct fpc1020_data *fpc1020)
{
	...
	if (!fpc1020) {
		pr_err("fpc %s: failed to allocate memory for struct fpc1020_data!\n",
		       __func__);
		return -ENOMEM;
	}
...
static int request_vreg_gpio(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->gpios_requested) {
		...
		if (rc) {
			...
			dev_dbg(dev, "fpc irq gpio applied at  %d\n",
				 fpc1020->irq_gpio);
		}

		dev_dbg(dev, "fpc irq gpio requested successfully!\n");
		...
		dev_dbg(dev, "fpc requested irq %d\n",
			 gpio_to_irq(fpc1020->irq_gpio));
	} else if (!enable && fpc1020->gpios_requested) {
		if (fpc1020->irq_requested) {
			devm_free_irq(dev, gpio_to_irq(fpc1020->irq_gpio),
				      fpc1020);
			dev_dbg(dev, "fpc irq free successfully!\n");
			fpc1020->irq_requested = false;
		}
...
release_irq_gpio:
		if (gpio_is_valid(fpc1020->irq_gpio)) {
			...
			fpc1020->irq_gpio = FPC_GPIO_NO_DEFAULT;
			dev_dbg(dev, "fpc irq gpio released successfully!\n");
		}

		fpc1020->gpios_requested = false;
	} else {
		dev_dbg(dev, "%s: enable: %d, gpios_requested: %d ???\n",
...
static int irq_setup(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable) {
		dev_dbg(dev, "fpc %s --->: enter, for enable irq!\n",
			 __func__);
	} else {
		dev_dbg(dev, "fpc %s --->: enter, for disable irq!\n",
			 __func__);
	}

	if (gpio_is_valid(fpc1020->irq_gpio)) {
		dev_dbg(dev, "fpc %s irq_gpio is validate!\n", __func__);

		if (enable) {
			enable_irq_wake(gpio_to_irq(fpc1020->irq_gpio));
			dev_dbg(dev, "fpc irq_gpio is enable.\n");
		} else {
			disable_irq(gpio_to_irq(fpc1020->irq_gpio));
			dev_dbg(dev, "fpc irq_gpio is disable.\n");
		}
	} else {
		dev_dbg(dev, "fpc %s irq_gpio is invalidate!\n", __func__);
		rc = -EINVAL;
	}
		      ...
		      bool enable)
{
	...
	if (enable) {
		dev_dbg(dev, "fpc %s --->: enter, for power on at %s.\n",
			 __func__, name);
	} else {
		dev_dbg(dev, "fpc %s --->: enter, for power off at %s.\n",
			 __func__, name);
	}
 ...
 * disabled.
 */
static ssize_t clk_enable_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable)
		dev_dbg(dev, "fpc %s --->: enter, for enable power!\n",
			 __func__);
	else
		dev_dbg(dev, "fpc %s --->: enter, for disable power!\n",
			 __func__);
	...
	if (enable && !fpc1020->prepared) {
		...
		if (power_cfg == 1) {
			...
			pr_debug("fp_vdd_vreg is enabled %d!\n",
...
static ssize_t power_cfg_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	int rc = 0;
	struct fpc1020_data *fpc1020 = dev_get_drvdata(dev);
	...
	dev_dbg(dev, "fpc set power_cfg: %d, rc: %d", power_cfg, rc);

	return rc ? rc : count;
}

static DEVICE_ATTR_WO(power_cfg);
...
static ssize_t vendor_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR_WO(vendor);
				 ...
				 void *data)
{
	...
	if (val != MI_DRM_BLANK_EVENT || fpc1020->prepared == false)
		return 0;

	pr_debug("[info] %s value = %d\n", __func__, (int)val);

	if (evdata && evdata->data && val == MI_DRM_BLANK_EVENT) {
		blank = *(int *)(evdata->data);
		switch (blank) {
		case MI_DRM_BLANK_POWERDOWN:
			fpc1020->fb_black = true;
			break;
		case MI_DRM_BLANK_ON:
